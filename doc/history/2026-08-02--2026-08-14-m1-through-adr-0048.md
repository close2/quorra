# M1 through ADR 0048 — 2026-08-02 to 2026-08-14

The record of every round from the first milestone to the retained encode, newest entry
first, moved here verbatim from `PLAN.md`'s "Where we are" on 2026-08-14. It is history:
some of it was superseded by a later entry in the same file (each such entry says so where
it was said at the time), and the decisions themselves live in `doc/adr/`. Nothing here is
edited after the fact — a correction is a later entry, which is what makes the file
readable as a record rather than as a claim about today.

**The convention this file starts:** a round adds a *new* file to `doc/history/`, named
`YYYY-MM-DD-slug.md`, and never edits an existing one. `PLAN.md`'s "Where we are" says
what is true now and is rewritten freely; this directory says what happened and is not.
