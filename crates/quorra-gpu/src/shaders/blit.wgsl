...
struct Placement {
    ...
    // textures of one size, which is what the hand-off and the seed are; the child's
    // rectangle inside its parent's, for the composite's backdrop (ADR 0038).
    origin: vec2f,
    _pad: vec2f,
}
...
fn fs_main(in: VsOut) -> @location(0) vec4f {
    return textureLoad(src_tex, vec2i(in.position.xy + placement.origin), 0);
}
